...
3. GPU device note: `pl.Trainer(accelerator="gpu", devices=[7], ...)`
   hard-codes GPU index 7. If your machine doesn't have at least 8 GPUs
   (indices 0-7), this will fail -- change `devices=[7]` to match an
   actual available GPU index on your machine (e.g. `devices=[0]`), or
   change `accelerator="gpu"` to `accelerator="cpu"` if you don't have a
   GPU at all (training will be much slower on CPU, but will still
   produce equivalent results given the same seed).
...
# `from utils import *` brings in helper functions/objects used below,
# including (based on how they're used further down) `get_R` (the Pearson
# correlation computation), `np` (numpy), and `sc` (scanpy) -- make sure
# `utils.py` actually provides all of these, or you'll see a `NameError`
# when running this file.
from utils import *

# `THItoGene` is the full, combined model architecture -- defined in a
# separate file, `vis_model.py`, which is not part of this project yet.
# This is presumably where the CapsNet, Graph Attention, and
# Omni-Dimensional Dynamic Convolution building blocks from earlier files
# in this project actually get assembled together into one model.
...
def train(test_sample_ID, vit_dataset, epochs, modelsave_address, dataset_name):
    ...
    # near the top of this file. `devices=[7]` hard-codes GPU index 7;
    # see reproducibility note #3 above if this doesn't match your own
    # machine's available GPUs.
